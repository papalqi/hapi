...
function resolvePlatformPackageName(platformName = platform, archName = arch) {
    try {
        const mainPkg = require('../package.json');
        if (mainPkg && typeof mainPkg.name === 'string' && mainPkg.name.length > 0) {
            return `${mainPkg.name}-${platformName}-${archName}`;
        }
    } catch (e) {
        // ignore
    }
    return `@twsxtd/hapi-${platformName}-${archName}`;
}

function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = resolvePlatformPackageName(platformName, archName);
